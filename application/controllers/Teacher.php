...
class Teacher extends CI_Controller
{
    ...
    /****MANAGE EXAM MARKS*****/
    function marks($exam_id = '', $class_id = '', $subject_id = '', $section_id = '')
    {
        if ($this->session->userdata('teacher_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['subject_id'] > 0 && $page_data['section_id'] > 0) {
                redirect(base_url() . 'index.php?teacher/marks/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['subject_id'] . '/' . $page_data['section_id'], 'refresh');
            } else {
        ...
        if ($this->input->post('operation') == 'update') {
            ...
            $this->session->set_flashdata('flash_message', 'Mark updated successfully!');
            
            redirect(base_url() . 'index.php?teacher/marks/' . $this->input->post('exam_id') . '/' . $this->input->post('class_id') . '/' . $this->input->post('subject_id') . '/' . $this->input->post('section_id'), 'refresh');
        }
	...
	/****** DAILY ATTENDANCE *****************/
    function manage_attendance($date='',$month='',$year='',$class_id='',$section_id='',$subject_id='')
    {
        if($this->session->userdata('teacher_login')!=1)redirect('login' , 'refresh');
        
        if($_POST)
        {
            // Loop all the students of $class_id and $section_id
            ...
            redirect(base_url() . 'index.php?teacher/manage_attendance/'.$date.'/'.$month.'/'.$year.'/'.$class_id.'/'.$section_id.'/'.$subject_id , 'refresh');
        }
        $page_data['date']     =    $date;
    ...
    {
        redirect(base_url() . 'index.php?teacher/manage_attendance/' .
