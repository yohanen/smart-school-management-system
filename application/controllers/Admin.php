...
/*	
 *	@author 	: Yohanen Dinagde
 *	date		: 15 March, 2025
 *	SIgnetBD
 *	yohanendinagde7@gmail.com
 */
...
class Admin extends CI_Controller
{
    ...
    function exam($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            $data['name']    = $this->input->post('name');
            $data['date']    = date('Y-m-d', strtotime($this->input->post('date'))); // Save as YYYY-MM-DD
    ...
    function marks($exam_id = '', $class_id = '', $section_id = '', $subject_id = '')
    {
        ...
        if ($this->input->post('operation') == 'selection') {
            ...
            if ($page_data['exam_id'] > 0 && $page_data['class_id'] > 0 && $page_data['section_id'] > 0 && $page_data['subject_id'] > 0) {
                redirect(base_url() . 'index.php?admin/marks/' . $page_data['exam_id'] . '/' . $page_data['class_id'] . '/' . $page_data['section_id'] . '/' . $page_data['subject_id'], 'refresh');
            } else {
                $this->session->set_flashdata('mark_message', 'Choose exam, class, section and subject');
        ...
        $page_data['page_title'] = 'View Exam Marks';
        $this->load->view('backend/index', $page_data);
    }
	...
	function manage_attendance($date = '', $month = '', $year = '', $class_id = '')
	{
        ...
        // Get from POST if available, else use current date
        $day = $this->input->post('day') ? $this->input->post('day') : ($date ? $date : date('d'));
        $month = $this->input->post('month') ? $this->input->post('month') : ($month ? $month : date('m'));
        $year = $this->input->post('year') ? $this->input->post('year') : ($year ? $year : date('Y'));
        $class_id = $this->input->post('class_id') ? $this->input->post('class_id') : $class_id;
        ...
        if ($class_id && $section_id) {
            ...
            foreach ($page_data['students'] as $student) {
                foreach ($page_data['subjects'] as $subject) {
                    ...
                    if ($attendance) {
                        $attendance_data[$student['student_id']][$subject['subject_id']] = $attendance;
                    }
                }
            }
    ...
    function get_sections($class_id) {
        ...
        foreach($sections as $row) {
            $options .= '<option value="'.$row['section_id'].'">Section '.$row['name'].'</option>';
        }
        echo $options;
    }
    ...
    function send_marks()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($exam_id > 0 && $class_id > 0 && $section_id > 0 && $student_id > 0) {
                ...
                $html = '<h2 style="text-align: center;">Student Marks Report</h2>';
                $html .= '<div style="text-align: center; margin-bottom: 20px;">' . $this->db->get_where('settings', array('type' => 'system_name'))->row()->description . '</div>';
                $html .= '<table style="width: 100%; margin-bottom: 15px;">';
                $html .= '<tr><td><strong>Student Name:</strong> ' . $student['name'] . '</td></tr>';
                $html .= '<tr><td><strong>Class:</strong> ' . $this->crud_model->get_class_name($class_id) . '</td></tr>';
                $html .= '<tr><td><strong>Exam:</strong> ' . $this->db->get_where('exam', array('exam_id' => $exam_id))->row()->name . '</td></tr>';
                $html .= '</table>';
                ...
                $html .= '<thead>
                    <tr style="background-color: #f5f5f5;">
                        ...
                        <th style="padding: 8px; text-align: center;">Marks Obtained</th>
                        <th style="padding: 8px; text-align: center;">Total Marks</th>
                ...
                foreach ($marks as $mark) {
                    $subject = $this->db->get_where('subject', array('subject_id' => $mark['subject_id']))->row()->name;
                    $grade = $this->crud_model->get_grade($mark['mark_obtained']);
                    $html .= '<tr>
                        <td style="padding: 8px;">' . $subject . '</td>
                        <td style="padding: 8px; text-align: center;">' . $mark['mark_obtained'] . '</td>
                        <td style="padding: 8px; text-align: center;">100</td>
                        <td style="padding: 8px; text-align: center;">' . $grade['name'] . '</td>
                        <td style="padding: 8px;">' . $mark['comment'] . '</td>
                    </tr>';
                    $total_marks += 100;
                ...
                $html .= '<tr style="background-color: #f9f9f9;">
                    ...
                    <td style="padding: 8px; text-align: center;"><strong>' . $total_obtained . '</strong></td>
                    <td style="padding: 8px; text-align: center;"><strong>' . $total_marks . '</strong></td>
                    <td style="padding: 8px; text-align: center;"><strong>' . number_format($percentage, 2) . '%</strong></td>
                ...
                // Generate PDF
                $pdf_file = 'uploads/marks_report_' . $student_id . '.pdf';
                $this->pdf->generate($html, FCPATH . $pdf_file);
                
                $pdf_file_url = base_url() . $pdf_file;
                $this->session->set_flashdata('flash_message', get_phrase('report_generated'));
            }
        }
        
        $page_data['page_name'] = 'send_marks';
        $page_data['page_title'] = 'Send Marks Report';
        $page_data['pdf_file_url'] = $pdf_file_url;
        $page_data['parent_phone'] = $parent_phone;
    ...
    function send_attendance()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($timestamp && $class_id > 0 && $section_id > 0) {
                ...
                foreach ($students as $student) {
                    ...
                    if ($parent && $parent->phone) {
                        ...
                        $html = $this->load->view('backend/admin/attendance_report_pdf', $data, true);
                        $pdf = $this->pdf->load();
                        $pdf->WriteHTML($html);
                        $pdf_file = 'uploads/attendance_report_' . $student['student_id'] . '.pdf';
                        $pdf->Output(FCPATH . $pdf_file, 'F');
                        
                        // Send via WhatsApp
                        $this->send_whatsapp_message($parent->phone, $pdf_file);
                    }
                }
                
                $this->session->set_flashdata('flash_message', get_phrase('reports_sent_successfully'));
            }
        }
        
        $page_data['page_name'] = 'send_attendance';
        $page_data['page_title'] = 'Send Attendance Report';
        $this->load->view('backend/index', $page_data);
    }
    
    private function send_whatsapp_message($phone, $pdf_file)
    {
        // WhatsApp API integration code here
        // This is a placeholder for the actual WhatsApp API implementation
    ...
    public function update_attendance() {
        ...
        // You may need to add date, class, section as needed for your schema
        $this->db->where('student_id', $student_id);
        $this->db->where('subject_id', $subject_id);
        // Add date, class, section as needed
    ...
    public function update_marks() {
        ...
        // Add exam_id, class_id, section_id, subject_id as needed
        $this->db->where('student_id', $student_id);
        // Add exam_id, class_id, section_id, subject_id as needed
        $this->db->update('mark', [
    ...
    function get_students($class_id, $section_id) {
        ...
        foreach($students as $row) {
            $options .= '<option value="'.$row['student_id'].'">'.$row['name'].' (Roll: '.$row['roll'].')</option>';
        }
        echo $options;
    }
    ...
    public function test_pdf()
    {
        ...
        // Add content
        $pdf->Cell(0, 10, 'Hello World! TCPDF is working!', 0, 1);
