<div class="dashboard-header-glass">
    <div class="dashboard-header-content">
        <div class="dashboard-header-logo">
            <i class="entypo-graduation-cap"></i>
        </div>
        <div>
            <div class="dashboard-header-title">Smart School Management System</div>
            <div class="dashboard-header-subtitle">Yohanen Dinagde University of <span class="highlight">TECHNOLOGY</span></div>
        </div>
    </div>
</div>
<!-- Raw Links -->
<div class="row">
	<!-- Raw Links -->
	<div class="col-md-12 col-sm-12 clearfix ">
		
        <ul class="list-inline links-list pull-left">
        <!-- Language Selector -->			
           <li class="dropdown language-selector">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown" data-close-others="true">
                        	<i class="entypo-user"></i> <?php echo $this->session->userdata('login_type');?>
                    </a>

				<?php if ($account_type != 'parent'):?>
				<ul class="dropdown-menu <?php if ($text_align == 'right-to-left') echo 'pull-right'; else echo 'pull-left';?>">
					<li>
						<a href="<?php echo base_url();?>index.php?<?php echo $account_type;?>/manage_profile">
                        	<i class="entypo-info"></i>
							<span><?php echo ('Edit Profile');?></span>
						</a>
					</li>
					<li>
						<a href="<?php echo base_url();?>index.php?<?php echo $account_type;?>/manage_profile">
                        	<i class="entypo-key"></i>
							<span><?php echo ('Change Password');?></span>
						</a>
					</li>
				</ul>
				<?php endif;?>
				<?php if ($account_type == 'parent'):?>
				<ul class="dropdown-menu <?php if ($text_align == 'right-to-left') echo 'pull-right'; else echo 'pull-left';?>">
					<li>
						<a href="<?php echo base_url();?>index.php?parents/manage_profile">
                        	<i class="entypo-info"></i>
							<span><?php echo ('Edit Profile');?></span>
						</a>
					</li>
					<li>
						<a href="<?php echo base_url();?>index.php?parents/manage_profile">
                        	<i class="entypo-key"></i>
							<span><?php echo ('Change Password');?></span>
						</a>
					</li>
				</ul>
				<?php endif;?>
			</li>
        </ul>
        
        
		<ul class="list-inline links-list pull-right">
			
			<!-- Language Selector 			
           <li class="dropdown language-selector">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown" data-close-others="true">
                        <i class="entypo-globe"></i> language
                    </a>
				
				<ul class="dropdown-menu <?php if ($text_align == 'left-to-right') echo 'pull-left'; else echo 'pull-right';?>">
					<?php
                            $fields = $this->db->list_fields('language');
                            foreach ($fields as $field)
                            {
                                if($field == 'phrase_id' || $field == 'phrase')continue;
                                ?>
                                    <li class="<?php if($this->session->userdata('current_language') == $field)echo 'active';?>">
                                        <a href="<?php echo base_url();?>index.php?multilanguage/select_language/<?php echo $field;?>">
                                            <img src="assets/images/flag/<?php echo $field;?>.png" style="width:16px; height:16px;" />	
												 <span><?php echo $field;?></span>
                                        </a>
                                    </li>
                                <?php
                            }
                            ?>
                    
				</ul>
				
			</li>
			-->
			<!--<li class="sep"></li>-->
			
			<li>
				<a href="<?php echo base_url();?>index.php?login/logout">
					Logout <i class="entypo-logout right"></i>
				</a>
			</li>
		</ul>
	</div>
	
</div>

<hr style="margin-top:0px;" />

<style>
.dashboard-header-glass {
    width: 100%;
    background: rgba(255,255,255,0.85);
    backdrop-filter: blur(8px);
    -webkit-backdrop-filter: blur(8px);
    border-bottom: 1px solid rgba(74, 108, 247, 0.08);
    box-shadow: 0 2px 12px rgba(74, 108, 247, 0.04);
    padding: 32px 0 18px 0;
    margin-bottom: 24px;
    animation: slideDown 0.7s cubic-bezier(0.4,0,0.2,1);
}
.dashboard-header-content {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
}
.dashboard-header-logo i {
    font-size: 38px;
    color: var(--primary-color, #4a6cf7);
    margin-bottom: 8px;
}
.dashboard-header-title {
    font-size: 2.1rem;
    font-weight: 700;
    color: #2d3436;
    letter-spacing: 0.5px;
    text-align: center;
}
.dashboard-header-subtitle {
    font-size: 1.1rem;
    color: #636e72;
    margin-top: 4px;
    text-align: center;
    font-weight: 400;
}
.dashboard-header-subtitle .highlight {
    color: var(--primary-color, #4a6cf7);
    font-weight: 700;
    letter-spacing: 1px;
}
@media (max-width: 600px) {
    .dashboard-header-title { font-size: 1.2rem; }
    .dashboard-header-subtitle { font-size: 0.95rem; }
    .dashboard-header-glass { padding: 18px 0 10px 0; }
}
@keyframes slideDown {
    from { opacity: 0; transform: translateY(-20px);}
    to { opacity: 1; transform: translateY(0);}
}
</style>